...
// v68: clear the one-shot-per-cast dead latches. Forward-declared because
// cgDeliverHit (far above the Lumos state section) also opens fresh windows.
...
// v67: Engine.Actor.TriggerEvent(Name EventName, Actor Other, Actor Instigator)
// is the engine's own event dispatch - what stock LumosTrigger's state machine
// calls to open a secret wall (TriggerEvent walks every actor whose Tag equals
// the event and calls its Trigger). Natives consume FFrame bytecode like every
// other exec thunk: NAME token (the 4-byte name index this engine uses - the
// runtime Spawn layout proves it), then two OBJ tokens. Resolved lazily by
// resolveLumosFields; absent on engines without the UE2 event helpers - the
// caller falls back to the manual Tag walk (lumosDispatchByTag).
static PFN_execActorFn g_execTriggerEvent = NULL;

static BOOL nativeTriggerEvent(void *actor, DWORD nameIdx, void *other,
                               void *instigator)
{
    if (!actor || !g_opsOK || g_opNameConst < 0 || !g_execTriggerEvent)
        return FALSE;
    BYTE bc[32]; int p = 0;
    bc[p++] = (BYTE)g_opNameConst;     *(DWORD *)(bc + p) = nameIdx; p += 4;
    bc[p++] = (BYTE)g_ops[OP_OBJ].op;  *(void **)(bc + p) = other; p += 4;
    bc[p++] = (BYTE)g_ops[OP_OBJ].op;  *(void **)(bc + p) = instigator; p += 4;
    bc[p++] = (BYTE)g_ops[OP_END].op;
    bc[p++] = (BYTE)g_ops[OP_END].op;   // guard
    FFrameLite st; memset(&st, 0, sizeof(st));
    st.Object = actor; st.Code = bc;
    DWORD result = 0;
    g_execTriggerEvent(actor, NULL, &st, &result);
    return TRUE;
}
...
// v60: the two charged projectiles need independent delivery watches. v59
// reused the holder's single ordinary-cast slot for only the last bonus (and
// cgTick did not tick slot 0 at all), leaving Harry's two bonus actors and one
// of P2's actors stationary at the caster as a permanent red glow.
static CgPending g_coopBonusPend[4];
...
static void cgResetLevel(void)
{
    ...
    memset(g_coopBonusPend, 0, sizeof(g_coopBonusPend));
...
static void cgDeliverHit(CgPending *p, BOOL spellAlive)
{
    ...
    if (did) {
        ...
        if (ciHas(p->targetName, "gargoyle") || ciHas(clsName, "Lumos")) {
            ...
            // v68: a delivered Lumos hit is always a fresh cast gesture -
            // re-arm every player's replicated light (the one-shot-per-cast
            // dead latch). Unconditional: a cast can land in the ~1 s gap
...
static BOOL cgInitPending(CgPending *p, int i, void *caster, void *spawned,
                          CgCand *c, void *spellCls, const char *kind)
{
    if (!p) return FALSE;
    memset(p, 0, sizeof(*p));
    p->spellSlot = -1;
    if (!c || !cgCandAlive(c)) {
        logf_("    [castgame] p%d %s target gone before the fire completed - no pending hit",
              i, kind ? kind : "cast");
        return FALSE;
    }
    ...
    if (p->spell && p->spellSlot < 0) {
        ...
        logf_("    [castgame] p%d %s spell actor not indexed - immediate hit on %s",
              i, kind ? kind : "cast", p->targetName);
        cgDeliverHit(p, TRUE);
        p->active = FALSE;
        return TRUE;
    }
    if (!p->spell) {
        ...
        logf_("    [castgame] p%d %s no spell actor - direct handler on %s",
              i, kind ? kind : "cast", p->targetName);
        cgDeliverHit(p, FALSE);
        p->active = FALSE;
        return TRUE;
    }
    logf_("    [castgame] p%d %s pending hit on %s: dist=%.0f speed=%.0f window=%lums hitR=%.0f",
          i, kind ? kind : "cast", p->targetName, d, speed,
          (unsigned long)flight, p->hitR);
    return TRUE;
}
...
static void cgArmHit(int i, void *caster, void *spawned, CgCand *c, void *spellCls)
{
    ...
    if (p->active)
        logf_("    [castgame] p%d previous pending hit on %s dropped (new cast)",
              i, p->targetName);
    cgInitPending(p, i, caster, spawned, c, spellCls, "cast");
}

static BOOL cgArmBonusHit(int i, int shot, void *caster, void *spawned,
                          CgCand *c, void *spellCls)
{
    CgPending *p = NULL;
    for (int k = 0; k < (int)(sizeof(g_coopBonusPend) /
                              sizeof(g_coopBonusPend[0])); k++) {
        if (!g_coopBonusPend[k].active) { p = &g_coopBonusPend[k]; break; }
    }
    // A new charge takes at least ten seconds, while a pending flight is
    // capped at 3.5 seconds, so this can only indicate corrupted/stale state.
    // Refuse to overwrite another live delivery: the projectile itself can
    // still collide naturally.
    if (!p) {
        logf_("    [coopcast] P%d bonus shot %d/3 has no free delivery watcher; "
              "leaving its game-native collision active", i + 1, shot);
        return FALSE;
    }
    char kind[40];
    _snprintf(kind, sizeof(kind) - 1, "charged bonus %d/3", shot);
    kind[sizeof(kind) - 1] = 0;
    return cgInitPending(p, i, caster, spawned, c, spellCls, kind);
}
...
// Per frame: watch one pending projectile; deliver when it reaches the target
// or the flight window closes with it still alive. A projectile that died on
// its own hit something naturally - the game already handled it.
static void cgTickPending(CgPending *p, DWORD now)
{
    ...
    if (!alive) {
        logf_("    [castgame] p%d spell %s ended on its own at +%lums (natural touch "
              "or expiry) - target %s", i, p->spellName, (unsigned long)age,
              p->targetName);
        p->active = FALSE;
        return;
    }
    ...
    BOOL targetOk = (g_objArray && p->targetSlot >= 0 &&
                     p->targetSlot < g_objArray->Num &&
                     g_objArray->Data[p->targetSlot] == p->target &&
                     !IsBadReadPtr(p->target, F.Location + 12) &&
                     (!p->tinfo || cgClassOf(p->target) == p->tinfo->cls));
    if (!targetOk) {
        logf_("    [castgame] p%d target %s vanished - pending hit dropped",
              i, p->targetName);
        p->active = FALSE;
        return;
    }
...
static void cgTick(void)
{
    ...
    DWORD now = GetTickCount();
    // P1's ordinary cast is engine-owned, hence ordinary slots begin at 1.
    for (int i = 1; i < 8; i++) cgTickPending(&g_cgPend[i], now);
    // Charged bonus shots are mod-owned for every holder, including P1.
    for (int k = 0; k < (int)(sizeof(g_coopBonusPend) /
                              sizeof(g_coopBonusPend[0])); k++)
        cgTickPending(&g_coopBonusPend[k], now);
}
...
static void coopQuietStockCompanionMotion(void *target)
{
    ...
    static void *sLoggedTarget[3] = { NULL, NULL, NULL };
    for (int p = 1; p <= 2; p++) {
        ...
        if (F.Velocity > 0 &&
            ...
            float *v = (float *)((BYTE *)pawn + F.Velocity);
            if (v[0] != 0.0f || v[1] != 0.0f) wrote = TRUE;
            v[0] = 0.0f; v[1] = 0.0f;
        }
        if (F.Acceleration > 0 &&
            !IsBadWritePtr((BYTE *)pawn + F.Acceleration, 12)) {
            float *a = (float *)((BYTE *)pawn + F.Acceleration);
            if (a[0] != 0.0f || a[1] != 0.0f) wrote = TRUE;
            a[0] = 0.0f; a[1] = 0.0f; // preserve gravity/jump acceleration
        }
...
static void coopObserveP1(void *p1, void *cursorTarget)
{
    ...
    // Keep the movement guard active across the same brief None-frame cursor
    // flicker that preserves the P1 hold. Otherwise stock AI gets an impulse
    // on every flicker even though the cooperative lock never ended.
    void *guardTarget = validTarget ? cursorTarget : g_p1CursorLockedTarget;
    if (guardTarget && cgLiveObject(guardTarget) &&
        actorInCurrentLevel(guardTarget) && !cgDeleted(guardTarget)) {
        void *guardClass = cgClassOf(guardTarget);
        if (guardClass && coopClassIsCooperative(guardClass))
            coopQuietStockCompanionMotion(guardTarget);
    }
    ...
    if (g_pelogOn) {
        if (GetTickCount() >= g_pelogEnd) {
            ...
            for (int p = 0; p < 8; p++) {
                ...
                // v70: name the Bump partner - the v69 capture showed a
                // companion Bump-ing something at a Lumos wall the mod had
                // "opened", without ever saying WHAT blocked her.
                char bb[100] = "";
                if (strstr(nb, ".Bump") && parms && !IsBadReadPtr(parms, 4)) {
                    void *o = *(void **)parms;
                    if (o && !IsBadReadPtr(o, 0x30) && cgClassOf(o)) {
                        char ob[80]; objName(o, ob, sizeof(ob));
                        _snprintf(bb, sizeof(bb) - 1, " other=%s", ob);
                        bb[sizeof(bb) - 1] = 0;
                    }
                }
                logf_("[pelog] p%d @+%lums %s%s", p,
                      (unsigned long)(now2 - g_pelogT0), nb, bb);
                g_pelogCount++;
                break;
            }
...
struct CoopCastActive {
    ...
    DWORD lastSeenAt;
    char name[160];
    // Stable per-arm candidate snapshot. fireCast used to discard its live
    // candidate when retaining the charged target, so even the holder's
    // ordinary P2 shot had no autohit watch. The same snapshot supplies both
    // independently tracked bonus deliveries, including metadata-less P1
    // cooperative targets.
    CgCand candidate;
    CoopCastHero hero[3];       // identity only in v59+
};
...
static CgCand *coopActiveCandidate(void)
{
    if (!g_coopActive.active) return NULL;
    CgCand *c = &g_coopActive.candidate;
    if (c->obj == g_coopActive.target.object &&
        c->slot == g_coopActive.target.slot && cgCandAlive(c))
        return c;
    // A scan refresh can replace g_cgCand while a charge is armed. Prefer a
    // fresh entry when available, but never accept a different object.
    c = cgFindCand(g_coopActive.target.object);
    return (c && cgCandAlive(c)) ? c : NULL;
}

// ===========================================================================
// v65: cooperative wet-shader "three floating spells" overlay, exact stock
// per-hero placement.
//
...
// PINNED them beside the target plane; v64 anchored both companions at the
// HOLDER's camera front point and nudged them sideways by a small fixed
// spacing, which collapsed the two icons onto each other whenever the
// companions' directions projected together. v65 drops that fabrication and
// reproduces the original per-hero recipe exactly:
...
//     (hp3trio::heroGoal): the point at the stock gesture distance on that
//     hero's own line of sight. The three heroes' lines fan out from the
//     target, so the three icons sit apart on their own heroes' sides;
//   * each gesture faces opposite its owner's line of sight (target -> hero,
//     hp3trio::facingYaw), not the shared pane-viewer yaw;
//   * each icon is chased with the stock stateLockedOn.Tick motion,
//     SpellGesture.MoveSmooth((goal - loc) * rate * fTimeDelta) - snapped
//     once at lock, then gliding in the air after its hero, rate 10 while
//     the target is still the current possible target, rate 8 on the cached
//     centre during the same 250 ms target flicker the hold tolerates (the
//     stock lock keeps its gesture while aCurrentTarget flickers);
//   * a companion hero that is unavailable for a frame keeps gliding toward
//     its last goal instead of being re-pinned or destroyed.
...
struct CoopTrioFX {
    ...
    // v65: each companion gesture belongs to a hero and moves the way the
    // stock SpellCursor moves that hero's OWN gesture (per-hero vLOS_End goal
    // + MoveSmooth chase + its own facing), instead of sharing the holder's
    // front point.
    ...
    float goal[2][3];            // last per-hero goal (flicker/hero-stale survival)
    BOOL  haveGoal[2];
    int   facing[2];             // last per-hero facing yaw
...
static void coopTrioUpdate(void)
{
    ...
    // v65: exact stock per-hero goal. Each joined hero owns one SpellGesture
    // whose goal is ITS OWN vLOS_End (target.Location + CentreOffset +
    // Normal(heroLoc - targetLoc) * fFinalGestureDistance) - the point at the
    // stock gesture distance on that hero's own line of sight, i.e.
    // hp3trio::heroGoal. The three heroes' lines fan out from the target, so
    // the three icons sit apart on their own heroes' sides and each glides
    // after its own hero instead of collapsing onto a shared front point.
    BOOL liveTarget = (cand != NULL);   // flicker = cand NULL (cached centre)
    float rate = liveTarget ? (float)hp3trio::kSmoothRate
                            : (float)hp3trio::kSmoothRateFlicker;
    ...
    for (int k = 0; k < 2; k++) {
        ...
        if (hp && coopActorAlive(hp) && F.Location > 0 &&
            !IsBadReadPtr((BYTE *)hp + F.Location, 12)) {
            float *hl = (float *)((BYTE *)hp + F.Location);
            float dirH[3];
            // Stock per-hero goal: centre + Normal(heroLoc - centre) * dist.
            if (hp3trio::direction(centre, hl, dirH)) {
                hp3trio::heroGoal(centre, hl, dist, goal);
                // Faces opposite this hero's line of sight: target -> hero.
                yawF = hp3trio::facingYaw(dirH);
                memcpy(t.goal[k], goal, 12);
                t.facing[k] = yawF;
                t.haveGoal[k] = TRUE;
                fresh = TRUE;
            }
        ...
        if (!fresh) {
            // Hero unavailable (or exactly at the aim centre) for a frame:
            // keep gliding toward the last goal, as the stock lock keeps its
            // gesture through a flicker. No captured goal means there is
            // nothing stock-consistent to draw for this hero.
            if (!t.haveGoal[k]) {
                coopTrioDisarm("companion hero unavailable");
                return;
            }
            memcpy(goal, t.goal[k], 12);
            yawF = t.facing[k];
        }
        ...
        // MoveSmooth((goal-loc)*rate*dt) - snapped once at lock, then the icon
        // floats in the air after its hero instead of teleporting to a pinned
        // side position. Rate 10 while the target is still a possible target,
        // 8 on the cached centre during a flicker.
        ...
        hp3trio::moveSmooth(t.cur[k], started, goal, rate, dt);
        t.curAt[k] = now;
                                      ...
                                      void *apparentCaster, void *sourceSpell)
{
    if (!pawn || !spawned || !candidate || !cgCandAlive(candidate) ||
        ...
        IsBadReadPtr((BYTE *)pawn + F.Location, 12) ||
        IsBadWritePtr((BYTE *)spawned + F.Location, 12) ||
        ...
        if (coopActorAlive(pawn) && g_fnSpawnSpell &&
            ...
            for (int shot = 2; shot <= 3; shot++) {
                ...
                *(void **)(parms + 0x04) = g_coopActive.target.object;
                ...
                // holder's untouched normal cast.
                if (spawned && !launched && !watched) {
                    BOOL gone = destroyActorFX(spawned);
                    logf_("  [coopcast] bonus shot %d/3 could not be launched or "
...
// The holder owns an ordinary P2/P3 aim that predates the charge, so normal
// restore does not stop it (its release must fire shot #1). Split shutdown has
// no controlled release, so cancel that original hold before returning its
// pawn to AI.
...
static BOOL coopStart(int holder, CgCand *candidate, const hp3coop::Target &key)
{
    ...
    g_coopActive.candidate = *candidate;
    // The P1 ad-hoc path resolves the object-table slot after constructing its
    // temporary candidate. Bind the snapshot to the validated hold identity.
    g_coopActive.candidate.obj = key.object;
    g_coopActive.candidate.slot = key.slot;
    g_coopActive.candidate.info = cgClassInfo(key.clazz);
    if (!g_coopActive.candidate.name[0])
        strncpy(g_coopActive.candidate.name, g_coopActive.name,
...
static void coopTick(int i, void *pawn, BOOL held)
{
    ...
    if (g_coopActive.active && g_coopActive.holder == i) {
        // Release is the charged fire moment, not cancellation: the holder's
        // normal path supplies shot #1 and coopRestore adds shots #2/#3. A
        // reticle flicker (<=250 ms) while held keeps the charge alive; only a
        // real target change/deletion past the grace cancels silently.
        if (!held) {
            coopRestore("holder released cast", TRUE);
...
// spawner therefore remains a normal single cast.
static void coopP1HoldTry(void *p1)
{
    ...
    if (!coopClassIsCooperative(cls)) {
        ...
        logf_("[coopcast] P1 >%lu-second cursor lock on %s: not the cooperative "
                "CompanionSpellTrigger class family - no charged x3 arm "
                "(ordinary holds stay a plain single-character cast)",
              (unsigned long)g_coopCastHoldMs / 1000u,
    ...
    if (!cand) {
        ...
        // an ad-hoc candidate; the class already passed the family gate
...
static void fireCast(int i, void *pawn)
{
    ...
    if (g_castGameplay) {
        ...
        if (g_coopActive.active && g_coopActive.holder == i &&
            ...
            CgCand *chargedCand = coopActiveCandidate();
            // Keep the live pick when it is already this target; otherwise use
            // the stable candidate captured at arm time. v59 unconditionally
            // nulled gameCand here, so P2's normal shot logged "target gone"
            // and lost its guaranteed delivery even though the target lived.
            if (!gameCand || gameCand->obj != g_coopActive.target.object)
                gameCand = chargedCand;
            gameTarget = g_coopActive.target.object;
            target = gameTarget;
            if (g_coopActive.spell && cgIsKnownClass(g_coopActive.spell))
                cls = g_coopActive.spell;
            logf_("  [coopcast] P%d ordinary release retains charged target %s "
                  "(delivery watch=%s)", i + 1, g_coopActive.name,
                  gameCand ? "armed" : "native-only");
        }
...
// v69: the Engine.Triggers family anchor (LumosTrigger/LumosSparkles both
// extend Engine.Triggers in the HP2 decompile HP3 inherits). A placed
// HP3-only trigger subclass with a look-alike token ("LumosTriggerLarge",
// "LumosSparklesEmitter"-adjacent names) can then join the trigger cache
// through a CHAIN POINTER PROOF against this anchor instead of being
// rejected by the exact-name rule and silently emptying the cache.
static void *g_clsTriggersFamily       = NULL;
// v69: the stock LumosLight.TurnOn/TurnOff script functions. Calling the
// light's OWN TurnOn via ProcessEvent replays the EXACT stock glow sequence
// as stock bytecode - TurnDynamicLightOn register, Enable('Tick'), the
// AllActors OnLumosOn broadcast, Particles = Spawn(Class'LumosLightFX',
// self,, Location), Particles.EnableEmission(True) - with zero dependency
// on the mod resolving LumosLightFX/Particles/bEmit offsets (the v66..v68
// hand-rolled spawn needed all of them and none is proven on hardware).
// TurnOff is the symmetric stock teardown. The only stock behaviour the
// call cannot do by itself is pass the single-player gate
// (if (PlayerHarry.bLumosOn) return - TurnOn reads the LEAD's flag through
// the light's own PlayerHarry var), so the caller masks that one bit around
// the call (see the v69 gate dance in lumosTick).
static void *g_fnLumosTurnOn           = NULL;
static void *g_fnLumosTurnOff          = NULL;
// v70: script Engine.Actor.TriggerEvent (see table).
static void *g_fnActorTriggerEvent     = NULL;
// v70: hgame.LumosLight.PlayerHarry - the light's OWN gate variable. Resolved
// by chain walk on the first live light; -2 = not tried, -1 = absent.
static int   g_offLightPlayerHarry     = -2;
...
// v69: Engine.Pawn.Weapon, kept SEPARATE from g_offWand (which prefers
// hgame.HPCharacter.Wand). Stock baseWand.Tick only assigns WandEndPoint -
// the vector UpdateLocation rides the light AND its Particles onto - inside
// `if ((Pawn(Owner) != None) && (Pawn(Owner).Weapon == self))`; a companion
// whose Weapon slot is empty makes the stock ride write (0,0,0) over the
// light+glow location every frame. Filling the empty slot with the companion's
// own wand actor re-enables the stock ride.
static int g_offWeapon           = -2;
...
// v67: defined below (after readLumosCollisionBits); getWandLumosLight's
// subclass acceptance needs it earlier in the file.
static BOOL lumosChainContainsClass(void *obj, void *want);
// ---------------------------------------------------------------------------
// v67: the rest of the stock Lumos register and the LumosTrigger state
// machine, resolved by name like every other script field. See the stock
// chain write-up in src/lumos_sync.h.
// ---------------------------------------------------------------------------
static int   g_offLightRadiusInner    = -2;        // Engine.Actor.LightRadiusInner
static BOOL  g_lightBrightIsFloat     = FALSE;     // layout tell: hue-brightness delta
static BOOL  g_lightRadiusInnerIsFloat= FALSE;     // adjacency to float LightRadius
static int   g_offLumosBOn            = -2;        // hgame.LumosLight.bLumosOn
static DWORD g_maskLumosBOn           = 0;
static int   g_offPawnLumosOn         = -2;        // PlayerHarry.bLumosOn (hgame.harry lineage)
static DWORD g_maskPawnLumosOn        = 0;
static int   g_offTrigFirstSent       = -2;        // hgame.LumosTrigger.bFirstEventSent
static DWORD g_maskTrigFirstSent      = 0;
static int   g_offTrigDist            = -2;        // hgame.LumosTrigger.fDistanceCheck (float)
static int   g_offTrigUseZ            = -2;        // hgame.LumosTrigger.bUseZDistanceCheck
static DWORD g_maskTrigUseZ           = 0;
static int   g_offTrigZDist           = -2;        // hgame.LumosTrigger.fZDistanceCheck (float)
static void *g_clsLumosLightFX        = NULL;      // HPParticle.LumosLightFX - the wand-glow particles
static int   g_offLightParticles      = -2;        // hgame.LumosLight.Particles - the glow the light relocates

// ---------------------------------------------------------------------------
// v68: DEFERRED LEVEL-SIDE RESOLUTION. resolveLumosFields used to be
// one-shot, and lumosTick runs EVERY rendered frame - including the MAIN
// MENU - so the first resolution happened before the hgame/HPParticle/
// level-side classes were loaded. The v66.5 hardware log proves it: the
// family classes logged 3 of 4 NULL (only the menu-loaded KWGame.
// GenericColObj resolved) and TheLumosLight=+0xFFFFFFFF - and every
// hgame-side field (bLumosOn, the trigger state machine, the glow class)
// was then cached NULL/-1 for the WHOLE session. That is exactly "the game
// doesn't seem to give them that state": the v67 replication had no glow
// class to spawn, no bLumosOn bit to write and no trigger fields to read.
// The engine-side fields above (Engine.Actor.*) exist from boot and stay
// one-shot; everything declared in hgame/HPParticle/KWGame now resolves in
// ONE combined table pass, retried at 1 Hz while anything is pending, so a
// menu-time first call can no longer poison the session.
// ---------------------------------------------------------------------------
// v70: LRK_CLASSTOKEN - path "*.Token" matches ANY "Class <pkg>.Token" whatever
// the package (HP3 moved LumosLightFX out of HPParticle: v69 log GlowFX=0).
enum LumosResKind { LRK_OBJECT = 0, LRK_OFFSET = 1, LRK_BOOL = 2, LRK_CLASSTOKEN = 3 };

struct LumosResEntry {
    const char *path;
    int         kind;
    void       *destObj;    // LRK_OBJECT: object pointer out
    int        *destOff;    // LRK_OFFSET / LRK_BOOL: property offset out
    DWORD      *destMask;   // LRK_BOOL: bitmask out
};

// Order matters for fallback chains: a later entry only ever writes when the
// destination is still unresolved (dest == NULL / offset == -2), so the
// first entry that CAN resolve wins (e.g. hgame.baseWand.TheLumosLight
// before hgame.HarryWand.TheLumosLight, HPParticle.LumosLightFX before the
// hgame-local copy).
static LumosResEntry g_lumosPending[] = {
    { "hgame.LumosLight",                    LRK_OBJECT, &g_clsLumosLight, NULL, NULL },
    { "hgame.LumosSpell",                    LRK_OBJECT, &g_clsLumosSpell, NULL, NULL },
    { "hgame.gargoyle",                      LRK_OBJECT, &g_clsGargoyle, NULL, NULL },
    { "hgame.LumosTrigger",                  LRK_OBJECT, &g_clsLumosTrigger, NULL, NULL },
    { "hgame.LumosSparklesTrigger",          LRK_OBJECT, &g_clsLumosSparklesTrigger, NULL, NULL },
    { "KWGame.GenericColObj",                LRK_OBJECT, &g_clsGenericColObj, NULL, NULL },
    { "KWGame.KWBlockingVolume",             LRK_OBJECT, &g_clsKWBlockingVolume, NULL, NULL },
    { "HPParticle.LumosLightFX",             LRK_OBJECT, &g_clsLumosLightFX, NULL, NULL },
    { "hgame.LumosLightFX",                  LRK_OBJECT, &g_clsLumosLightFX, NULL, NULL },
    { "*.LumosLightFX",                      LRK_CLASSTOKEN, &g_clsLumosLightFX, NULL, NULL },
    // v70: the SCRIPT Engine.Actor.TriggerEvent (UE2: a plain UnrealScript
    // function - ForEach DynamicActors(class'Actor', A, EventName)
    // A.Trigger(Other, Instigator); the HP2 decompile's DuelTrigger overrides
    // exactly this signature). Called through ProcessEvent the ENGINE does
    // the virtual dispatch of each receiver's Trigger - state-declared
    // Mover/trigger Triggers included - which the mod's own Tag walk could
    // not (v69 log: it ran the EMPTY Engine.Actor.Trigger on
    // LumosSparklesTrigger5 and called that a fire).
    { "Engine.Actor.TriggerEvent",           LRK_OBJECT, &g_fnActorTriggerEvent, NULL, NULL },
    { "hgame.baseWand.TheLumosLight",        LRK_OFFSET, NULL, &g_offTheLumosLight, NULL },
    { "hgame.HarryWand.TheLumosLight",       LRK_OFFSET, NULL, &g_offTheLumosLight, NULL },
    { "hgame.HPCharacter.Wand",              LRK_OFFSET, NULL, &g_offWand, NULL },
    { "Engine.Pawn.Weapon",                  LRK_OFFSET, NULL, &g_offWand, NULL },
    { "hgame.LumosLight.Particles",          LRK_OFFSET, NULL, &g_offLightParticles, NULL },
    // v69: the stock TurnOn/TurnOff SCRIPT functions. TurnOn replays the
    // whole stock glow sequence as stock bytecode (register, Enable('Tick'),
    // OnLumosOn broadcast, Particles spawn + EnableEmission) - the primary
    // companion-glow path; the v68 hand-rolled spawn stays as the fallback
    // when these fail to resolve.
    { "hgame.LumosLight.TurnOn",             LRK_OBJECT, &g_fnLumosTurnOn, NULL, NULL },
    { "hgame.LumosLight.TurnOff",            LRK_OBJECT, &g_fnLumosTurnOff, NULL, NULL },
    { "hgame.LumosLight.bLumosOn",           LRK_BOOL,   NULL, &g_offLumosBOn, &g_maskLumosBOn },
    { "hgame.harry.bLumosOn",                LRK_BOOL,   NULL, &g_offPawnLumosOn, &g_maskPawnLumosOn },
    { "hgame.HPawn.bLumosOn",                LRK_BOOL,   NULL, &g_offPawnLumosOn, &g_maskPawnLumosOn },
    { "hgame.HPCharacter.bLumosOn",          LRK_BOOL,   NULL, &g_offPawnLumosOn, &g_maskPawnLumosOn },
    { "hgame.LumosTrigger.bFirstEventSent",  LRK_BOOL,   NULL, &g_offTrigFirstSent, &g_maskTrigFirstSent },
    { "hgame.LumosTrigger.fDistanceCheck",   LRK_OFFSET, NULL, &g_offTrigDist, NULL },
    // v69: the Engine.Triggers family anchor for subclass chain proofs
    // (LumosTrigger / LumosSparkles both `extends Triggers` in the HP2
    // decompile HP3 inherits). In the retried table on purpose: a one-shot
    // engine-block lookup would repeat the menu-time miss v68 just fixed.
    { "Engine.Triggers",                     LRK_OBJECT, &g_clsTriggersFamily, NULL, NULL },
    { "hgame.LumosTrigger.bUseZDistanceCheck", LRK_BOOL, NULL, &g_offTrigUseZ, &g_maskTrigUseZ },
    { "hgame.LumosTrigger.fZDistanceCheck",  LRK_OFFSET, NULL, &g_offTrigZDist, NULL },
};

// UBoolProperty::BitMask sits right after UProperty::Offset - the same scan
// boolBitMask() does, but on an already-found property object (the combined
// pass must not re-scan the whole table per bool).
static DWORD lumosBoolMaskOf(void *propObj)
{
    if (!propObj || g_propOffsetField < 0 ||
        IsBadReadPtr(propObj, g_propOffsetField + 0x40)) return 0;
    for (int d = 4; d <= 0x38; d += 4) {
        DWORD m = *(DWORD *)((BYTE *)propObj + g_propOffsetField + d);
        if (m && (m & (m - 1)) == 0) return m;
    }
    return 1;
}

static BOOL lumosResPendingCount(void)
{
    int pend = 0;
    for (size_t i = 0; i < sizeof(g_lumosPending) / sizeof(g_lumosPending[0]); i++) {
        LumosResEntry *e = &g_lumosPending[i];
        switch (e->kind) {
        case LRK_OBJECT:
        case LRK_CLASSTOKEN: if (e->destObj && *(void **)e->destObj == NULL) pend++; break;
        default:             if (e->destOff && *e->destOff == -2)            pend++; break;
        }
    }
    return pend > 0;
}

// One combined table pass: resolves every still-pending level-side entry
// with a single GetFullName walk (objName), never one scan per path.
static void lumosResolvePendingFields(void)
{
    if (!lumosResPendingCount()) return;
    if (!g_objArray || IsBadReadPtr(g_objArray, sizeof(TArrayLite))) return;
    int n = g_objArray->Num;
    if (n <= 0 || n > 400000 || IsBadReadPtr(g_objArray->Data, 4)) return;
    char buf[256];
    for (int i = 0; i < n; i++) {
        ...
        if (!sp) continue;
        const char *full = sp + 1;
        for (size_t e = 0; e < sizeof(g_lumosPending) / sizeof(g_lumosPending[0]); e++) {
            LumosResEntry *en = &g_lumosPending[e];
            if (en->kind == LRK_CLASSTOKEN) {
                // "*.Token": a Class object whose last path component is Token.
                if (strncmp(buf, "Class ", 6)) continue;
                const char *dot = strrchr(full, '.');
                if (!dot || strcmp(dot + 1, en->path + 2)) continue;
                if (en->destObj && *(void **)en->destObj == NULL) {
                    *(void **)en->destObj = o;
                    logf_("[lumos] v70 class token %s resolved to %s", en->path, full);
                }
                break;
            }
            if (strcmp(full, en->path)) continue;
            switch (en->kind) {
            case LRK_OBJECT:
                if (en->destObj && *(void **)en->destObj == NULL)
                    *(void **)en->destObj = o;
            ...
            case LRK_CLASSTOKEN:
                break;
            case LRK_OFFSET:
                if (en->destOff && *en->destOff == -2 && g_propOffsetField >= 0 &&
                    !IsBadReadPtr(o, g_propOffsetField + 4)) {
                    int off = (int)*(DWORD *)((BYTE *)o + g_propOffsetField);
                    if (off > 0) *en->destOff = off;
                }
                break;
            case LRK_BOOL:
                if (en->destOff && *en->destOff == -2 && g_propOffsetField >= 0 &&
                    ...
                    int off = (int)*(DWORD *)((BYTE *)o + g_propOffsetField);
                    if (off > 0) {
                        DWORD mask = lumosBoolMaskOf(o);
                        if (!mask) mask = 1;
                        *en->destOff = off;
                        if (en->destMask) *en->destMask = mask;
                    }
                }
                break;
            }
            break;   // a path matches at most one object
        }
    }
}
...
{
    static BOOL sEngineResolved = FALSE;
    // ---- engine-side fields: exist from boot, resolve exactly once ----
    if (!sEngineResolved) {
        sEngineResolved = TRUE;
        ...
        g_offLightRadiusInner= propOffset("Engine.Actor.LightRadiusInner");
        g_lightBrightIsFloat = hp3lumos::brightnessIsFloatByLayout(g_offLightBrightness,
                                                                   g_offLightHue);
        g_lightRadiusInnerIsFloat = hp3lumos::radiusInnerIsFloatByLayout(
                                                                   g_offLightRadius,
                                                                   g_offLightRadiusInner);
        ...
        // v67: the engine's own event dispatch (stock LumosTrigger opens
        // walls with it). UE2-style engines export the exec thunk; if this
        // hybrid does not, lumosDispatchByTag is the fallback dispatcher.
        if (!g_execTriggerEvent) {
            g_execTriggerEvent = (PFN_execActorFn)resolveThunk((BYTE *)GetProcAddress(
                g_engine, "?execTriggerEvent@AActor@@QAEXAAUFFrame@@QAX@Z"));
            if (!g_execTriggerEvent)
                g_execTriggerEvent = (PFN_execActorFn)resolveThunk((BYTE *)GetProcAddress(
                    g_engine, "?execTriggerEvent@AActor@@UAEXAAUFFrame@@QAX@Z"));
        }
        ...
        // v69: Engine.Pawn.Weapon, separate from the Wand offset. Same
        // reliability class as Tag/Event here: the engine block runs on the
        // first rendered frame, when GObjObjects is long since live (that is
        // where g_offTag/g_offEvent have always resolved on hardware).
        g_offWeapon          = propOffset("Engine.Pawn.Weapon");

        logf_("[lumos] resolved: LightType=+0x%X LightBrightness=+0x%X "
              "bCollideActors=+0x%X(0x%lX) bHidden=+0x%X(0x%lX) Owner=+0x%X "
              "Tag=+0x%X Event=+0x%X",
              g_offLightType, g_offLightBrightness,
              g_offbCollideActors, (unsigned long)g_maskbCollideActors,
              g_offbHiddenLumos, (unsigned long)g_maskbHiddenLumos, g_offOwner,
              g_offTag, g_offEvent);
    }

    // ---- level-side fields: deferred, one combined pass at 1 Hz -----------
    static DWORD sLastTry = 0;
    static int   sAttempts = 0;
    DWORD now = GetTickCount();
    if ((DWORD)(now - sLastTry) >= hp3lumos::ResolveRetryMs) {
        sLastTry = now;
        lumosResolvePendingFields();
        sAttempts++;
    }
    static BOOL sAllResolvedLogged = FALSE;
    static int  sLoggedAttempt = -1;
    BOOL pending = lumosResPendingCount();
    // v70: the v69 condition was evaluated EVERY FRAME (sAttempts only
    // advances at 1 Hz), so the PARTIAL line flooded the log ~60x/s for a
    // whole second every 30 s. Log once per resolution attempt at most.
    if (!sAllResolvedLogged && sLoggedAttempt != sAttempts &&
        (!pending || (sAttempts % 30) == 0)) {
        sLoggedAttempt = sAttempts;
        if (!pending) sAllResolvedLogged = TRUE;
        // v68: one line for the hardware log - every field the replication
        // depends on, so a field report shows immediately what is reachable.
        // v69: + Pawn.Weapon, the stock TurnOn/TurnOff functions and the
        // Engine.Triggers family anchor.
        logf_("[lumos] v69 level-side fields %s: Wand=+0x%X PawnWeapon=+0x%X "
              "TheLumosLight=+0x%X LumosLight.Particles=+0x%X "
              "brightnessIsFloat=%d radiusInnerIsFloat=%d "
              "LumosLight.bLumosOn=+0x%X(0x%lX) "
              "Pawn.bLumosOn=+0x%X(0x%lX) Trig.bFirstEventSent=+0x%X(0x%lX) "
              "Trig.fDistanceCheck=+0x%X Trig.bUseZ=+0x%X(0x%lX) "
              "Trig.fZDistanceCheck=+0x%X execTriggerEvent=%p GlowFX=%p "
              "TurnOn=%p TurnOff=%p",
              pending ? "PARTIAL (retrying at 1 Hz)" : "resolved",
              g_offWand, g_offWeapon, g_offTheLumosLight, g_offLightParticles,
              g_lightBrightIsFloat ? 1 : 0, g_lightRadiusInnerIsFloat ? 1 : 0,
              g_offLumosBOn, (unsigned long)g_maskLumosBOn,
              g_offPawnLumosOn, (unsigned long)g_maskPawnLumosOn,
              g_offTrigFirstSent, (unsigned long)g_maskTrigFirstSent,
              g_offTrigDist, g_offTrigUseZ, (unsigned long)g_maskTrigUseZ,
              g_offTrigZDist, g_execTriggerEvent, g_clsLumosLightFX,
              g_fnLumosTurnOn, g_fnLumosTurnOff);
        logf_("[lumos] v70 script TriggerEvent=%p "
              "Pawn.bLumosOn(chain)=+0x%X(0x%lX) Light.PlayerHarry=+0x%X",
              g_fnActorTriggerEvent,
              g_offPawnLumosOn, (unsigned long)g_maskPawnLumosOn,
              g_offLightPlayerHarry);
        // v66.4: the family class anchor pointers the exact/chain membership
        // proofs compare against - visible in the log so a hardware dump
        // shows immediately whether subclass proofs were even possible.
        // v69: + Engine.Triggers (the trigger-family chain anchor).
        logf_("[lumos] family classes: LumosTrigger=%p LumosSparklesTrigger=%p "
              "GenericColObj=%p KWBlockingVolume=%p LumosLight=%p "
              "Engine.Triggers=%p",
              g_clsLumosTrigger, g_clsLumosSparklesTrigger,
              g_clsGenericColObj, g_clsKWBlockingVolume, g_clsLumosLight,
              g_clsTriggersFamily);
    }
}
    ...
    if (wand && g_objArray && g_objArray->Data && g_clsLumosLight &&
        ...
        if (n > 0 && n <= 400000 && !IsBadReadPtr(g_objArray->Data, 4)) {
            for (int k = 0; k < n; k++) {
                void *o = g_objArray->Data[k];
                if (!o || IsBadReadPtr(o, 0x100)) continue;
                // v67: exact class, or a chain-proven LumosLight subclass.
                if (cgClassOf(o) != g_clsLumosLight &&
                    !lumosChainContainsClass(o, g_clsLumosLight)) continue;
...
// v67: the old isLumosActorHidden() reader is gone with its only caller -
// the v66.5 inactive branch that hid companion lights. Lights the mod turned
// on now get the exact stock TurnOff register instead of a hide.
...
#define LUMOS_MAX_ACTORS 1024   // v67: hoisted above the per-player state

// v68: GetTickCount() when a failed trigger dispatch may be retried (a fire
// that found no receiver no longer latches the once-per-level flag - it
// retries at 1 Hz so a wall linkage that appears late is not lost). Hoisted
// here because lumosFireTriggerEvent (defined below) uses it.
static DWORD g_lumosTrigNextTryAt[LUMOS_MAX_ACTORS] = { 0 };

// ---------------------------------------------------------------------------
// v67: stock-faithful per-player Lumos replication helpers. Everything here
// replays a concrete stock-script effect (see the stock chain write-up in
// src/lumos_sync.h); nothing invents behaviour the game does not have.
// ---------------------------------------------------------------------------

// ---------------------------------------------------------------------------
// v67 per-player replication state. Stock gives the Lumos state ONLY to
// PlayerHarry; these carry the replicated copy for every driven player.
// ---------------------------------------------------------------------------
// TRUE while the mod has applied the stock TurnOn register to that player's
// own TheLumosLight. Only lights with this latch are ever turned off by the
// mod - a stock-burning light (PlayerHarry's, or an infinite gargoyle's)
// stays in the game's hands.
static BOOL   g_lumosLightOn[8]   = { FALSE };
// v69: TRUE when that player's light was lit by calling the STOCK
// LumosLight.TurnOn script (which arms the light's own Tick - the stock 30 s
// auto-off, particle scaling and OnLumosOff broadcast then run stock-side).
// Stock-lit lights are never off-timed or off-registered by the mod; only
// fallback-lit (hand-rolled v68 register) lights keep the mod timer.
static BOOL   g_lumosLightStock[8] = { FALSE };
// GetTickCount() when the mod turned the light on; the stock LumosLight.Tick
// auto-offs after 30 s (fLumosTimeToTurnOff) - the mod replays that here,
// otherwise the mod's own light-follow ratchet would keep the window open
// forever (the mod keeps the light burning while the state is active).
static DWORD  g_lumosLightOnAt[8] = { 0 };
// v68: TRUE once this player's replicated light hit the stock 30 s auto-off
// (or was turned off by an expired state). The stock window is ONE SHOT per
// cast: without this latch the light-follow ratchet still kept the state
// alive for ~1 s after the auto-off, and the still-active state would turn
// the light straight back on - the replicated Lumos never ended. The latch
// clears only on a NEW Lumos window (state inactive -> active), i.e. a
// fresh cast.
...
static signed char g_lumosTrigFired[LUMOS_MAX_ACTORS];
...
static float lumosReadFloat(void *base, int off, float def)
{
    if (!base || off <= 0 || IsBadReadPtr((BYTE *)base + off, 4)) return def;
    return *(float *)((BYTE *)base + off);
}

static unsigned char lumosReadByte(void *base, int off, unsigned char def)
{
    if (!base || off <= 0 || IsBadReadPtr((BYTE *)base + off, 1)) return def;
    return *(BYTE *)((BYTE *)base + off);
}

static BOOL lumosReadBit(void *base, int off, DWORD mask, BOOL def)
{
    if (!base || off <= 0 || !mask || IsBadReadPtr((BYTE *)base + (off & ~3), 4))
        return def;
    return (*(DWORD *)((BYTE *)base + (off & ~3)) & mask) != 0;
}

static BOOL lumosWriteBit(void *base, int off, DWORD mask, BOOL on)
{
    if (!base || off <= 0 || !mask || IsBadWritePtr((BYTE *)base + (off & ~3), 4))
        return FALSE;
    DWORD *p = (DWORD *)((BYTE *)base + (off & ~3));
    if (on) *p |= mask;
    else    *p &= ~mask;
    return TRUE;
}

// Type-aware brightness read: a float-brightness engine read as a byte shows
// 0x00 for a burning 400.0f light (the v66.5 detection blindness).
static float lumosBrightnessOf(void *light)
{
    if (!light || g_offLightBrightness <= 0) return 0.0f;
    if (g_lightBrightIsFloat)
        return lumosReadFloat(light, g_offLightBrightness, 0.0f);
    return (float)lumosReadByte(light, g_offLightBrightness, 0);
}

// "Is this wand light genuinely burning" - the stock bLumosOn flag when it is
// reachable, else the dynamic-light register the stock TurnOn/Off writes.
static BOOL lumosLightBurning(void *light)
{
    if (!light) return FALSE;
    if (g_offLumosBOn > 0 && g_maskLumosBOn)
        return lumosReadBit(light, g_offLumosBOn, g_maskLumosBOn, FALSE);
    BYTE lt = lumosReadByte(light, g_offLightType, 0);
    return hp3lumos::isLightActiveF(FALSE, lt, lumosBrightnessOf(light)) ? TRUE : FALSE;
}

// Write the exact stock LumosLight.TurnDynamicLightOn/TurnDynamicLightOff
// register. writeBOn additionally writes the bLumosOn flag (the stock
// TurnOn/TurnOff maintain it) - the v68 turn-on sequence writes the flag
// SEPARATELY, only after the glow exists, because the stock wand Tick
// starts riding the light the moment bLumosOn reads True. The caller never
// touches the light's collision - stock spawns it
// SetCollision(False,False,False) and its Touch is an empty singular.
static void lumosApplyLightRegister(void *light,
                                    const hp3lumos::LumosLightRegister &r,
                                    BOOL brightnessIsFloat,
                                    BOOL radiusInnerIsFloat, BOOL on,
                                    BOOL writeBOn)
{
    if (!light) return;
    if (g_offLightType > 0 && !IsBadWritePtr((BYTE *)light + g_offLightType, 1))
        *(BYTE *)((BYTE *)light + g_offLightType) = r.lightType;
    if (g_offLightEffect > 0 && !IsBadWritePtr((BYTE *)light + g_offLightEffect, 1))
        *(BYTE *)((BYTE *)light + g_offLightEffect) = r.lightEffect;
    if (g_offLightBrightness > 0) {
        if (brightnessIsFloat) {
            if (!IsBadWritePtr((BYTE *)light + g_offLightBrightness, 4))
                *(float *)((BYTE *)light + g_offLightBrightness) = r.brightnessFloat;
        } else if (!IsBadWritePtr((BYTE *)light + g_offLightBrightness, 1)) {
            *(BYTE *)((BYTE *)light + g_offLightBrightness) = r.brightnessByte;
        }
    }
    if (g_offLightHue > 0 && !IsBadWritePtr((BYTE *)light + g_offLightHue, 1))
        *(BYTE *)((BYTE *)light + g_offLightHue) = r.hue;
    if (g_offLightSaturation > 0 && !IsBadWritePtr((BYTE *)light + g_offLightSaturation, 1))
        *(BYTE *)((BYTE *)light + g_offLightSaturation) = r.saturation;
    if (g_offLightRadius > 0 && !IsBadWritePtr((BYTE *)light + g_offLightRadius, 4))
        *(float *)((BYTE *)light + g_offLightRadius) = r.radius;
    if (radiusInnerIsFloat && g_offLightRadiusInner > 0 &&
        !IsBadWritePtr((BYTE *)light + g_offLightRadiusInner, 4))
        *(float *)((BYTE *)light + g_offLightRadiusInner) = r.radiusInner;
    if (writeBOn && g_offLumosBOn > 0 && g_maskLumosBOn)
        lumosWriteBit(light, g_offLumosBOn, g_maskLumosBOn, on);
}

// hgame.LumosLight.Particles - the glow actor the light's own UpdateLocation
// relocates with the light itself. Stock TurnOn stores the spawned
// LumosLightFX here and the wand's Tick then calls
// TheLumosLight.UpdateLocation(WandEndPoint) every frame while bLumosOn,
// which SetLocations BOTH the light and its Particles - the glow rides the
// wand tip through this pointer. Without it the mod's glow is a dead
// particle heap at the spawn point.
static void lumosSetLightParticles(void *light, void *fx)
{
    if (!light || g_offLightParticles <= 0 ||
        IsBadWritePtr((BYTE *)light + g_offLightParticles, 4)) return;
    *(void **)((BYTE *)light + g_offLightParticles) = fx;
}

// Resolve a property declared somewhere in the object's OWN class chain
// (most-derived first). TheLumosLight is declared on baseWand - the fixed
// "hgame.baseWand.TheLumosLight" path missed on hardware
// (log: +0xFFFFFFFF) - so the declaring class is found by walking the
// actual chain instead of guessing a name.
static int lumosPropOffForChain(void *obj, const char *prop, char *foundOn,
                                int foundOnCch)
{
    if (foundOn && foundOnCch > 0) foundOn[0] = 0;
    if (!obj || !prop) return -1;
    char cbuf[160];
    void *c = cgClassOf(obj);
    for (int d = 0; c && d < 12; d++, c = cgSuper(c)) {
        objName(c, cbuf, sizeof(cbuf));
        const char *sp = strchr(cbuf, ' ');
        if (!sp) break;
        char path[192];
        sprintf(path, "%s.%s", sp + 1, prop);
        int off = propOffset(path);
        if (off > 0) {
            if (foundOn && foundOnCch > 0) {
                strncpy(foundOn, sp + 1, foundOnCch - 1);
                foundOn[foundOnCch - 1] = 0;
            }
            return off;
        }
    }
...
// Write the FX's own bEmit bit (ParticleFX.bEmit): stock TurnOn follows the
// Spawn with Particles.EnableEmission(True), and StartGlowingWand writes
// bEmit = True directly - the property write is the stock-approved path.
static BOOL lumosSetFXEmit(void *fx, BOOL on)
{
    if (!fx) return FALSE;
    char foundOn[160];
    int off = lumosPropOffForChain(fx, "bEmit", foundOn, sizeof(foundOn));
    if (off <= 0) return FALSE;
    char path[192];
    sprintf(path, "%s.bEmit", foundOn);
    DWORD mask = boolBitMask(path);
    if (!mask) mask = 1;
    if (IsBadWritePtr((BYTE *)fx + (off & ~3), 4)) return FALSE;
    DWORD *slot = (DWORD *)((BYTE *)fx + (off & ~3));
    if (on) *slot |= mask;
    else    *slot &= ~mask;
    return TRUE;
}

// The TheLumosLight property is declared on the wand's class (HP3 save chain:
// "Class hgame HarryWand TheLumosLight[0]"), so resolve it from the wand's own
// class token and walk up the chain for the declaring ancestor. The fixed
// "hgame.baseWand/hgame.HarryWand" attempts both missed on hardware
// (log: TheLumosLight=+0xFFFFFFFF). Cached per wand class.
static int lumosLightPropOffForWand(void *wand)
{
    if (!wand) return -1;
    void *cls = cgClassOf(wand);
    if (!cls) return -1;
    for (int k = 0; k < 8; k++)
        if (g_wandLightProp[k].cls == cls) return g_wandLightProp[k].off;
    char foundOn[160];
    int off = lumosPropOffForChain(wand, "TheLumosLight", foundOn, sizeof(foundOn));
    if (off > 0)
        logf_("[lumos] v68 TheLumosLight property found on %s = +0x%X",
              foundOn, off);
    for (int k = 0; k < 8; k++) {
        if (!g_wandLightProp[k].cls) {
            g_wandLightProp[k].cls = cls;
            g_wandLightProp[k].off = off;
            break;
        }
    }
    return off;
...
// v70: the pawn's bLumosOn bit resolved from the LEAD PAWN'S OWN class chain.
// The v69 table guessed hgame.harry / hgame.HPawn / hgame.HPCharacter and
// every guess missed on HP3 (v69 log: Pawn.bLumosOn=+0xFFFFFFFE) - HP3
// renamed the hierarchy (hgame.HPPawn, hgame.HPHeroPawn, ...). With the bit
// unresolved the v69 gate dance masked nothing, stock TurnOn early-returned
// on PlayerHarry.bLumosOn (the lead was already lit by the gargoyle) and
// the companion light never engaged. Same chain walk as TheLumosLight.
static void lumosResolvePawnLumosBit(void *pawn)
{
    if (g_offPawnLumosOn != -2 || !pawn) return;
    char foundOn[160];
    int off = lumosPropOffForChain(pawn, "bLumosOn", foundOn, sizeof(foundOn));
    if (off <= 0) {
        // keep -2 so a later (correct) pawn can still resolve it; log once
        static BOOL sSaid = FALSE;
        if (!sSaid) {
            sSaid = TRUE;
            char nb[128];
            logf_("[lumos] v70 bLumosOn not declared anywhere on %s's chain "
                  "(retrying on other pawns)", objName(pawn, nb, sizeof(nb)));
        }
        return;
    }
    char path[192];
    sprintf(path, "%s.bLumosOn", foundOn);
    DWORD mask = nativeBoolBitMask(path);
    if (!mask) mask = boolBitMask(path);
    if (!mask) mask = 1;
    g_offPawnLumosOn = off;
    g_maskPawnLumosOn = mask;
    logf_("[lumos] v70 Pawn.bLumosOn resolved on %s = +0x%X mask 0x%lX",
          foundOn, off, (unsigned long)mask);
}

// v70: does THIS pawn's own class chain declare bLumosOn at the resolved
// offset? The lead (harry) and a companion (Hermione/Ron) may not share the
// declaring class - stock declares it on harry - and writing the bit (or
// letting the retargeted stock TurnOn write PlayerHarry.bLumosOn) at harry's
// offset inside a Hermione object would poke a foreign field. Cached per
// class; the chain walk costs a table scan per ancestor.
static BOOL lumosPawnHasLumosBit(void *pawn)
{
    if (!pawn || g_offPawnLumosOn <= 0) return FALSE;
    static struct { void *cls; BOOL has; } sCache[8];
    void *cls = cgClassOf(pawn);
    if (!cls) return FALSE;
    for (int k = 0; k < 8; k++)
        if (sCache[k].cls == cls) return sCache[k].has;
    char foundOn[160];
    int off = lumosPropOffForChain(pawn, "bLumosOn", foundOn, sizeof(foundOn));
    BOOL has = (off == g_offPawnLumosOn);
    char nb[128];
    logf_("[lumos] v70 %s: bLumosOn %s (+0x%X vs resolved +0x%X)",
          objName(pawn, nb, sizeof(nb)),
          has ? "declared on its chain" : "NOT on its chain - bit never written",
          off, g_offPawnLumosOn);
    for (int k = 0; k < 8; k++)
        if (!sCache[k].cls) { sCache[k].cls = cls; sCache[k].has = has; break; }
    return has;
}

// v70: hgame.LumosLight.PlayerHarry (object ref) on the light's own chain.
static void lumosResolveLightPlayerHarry(void *light)
{
    if (g_offLightPlayerHarry != -2 || !light) return;
    char foundOn[160];
    int off = lumosPropOffForChain(light, "PlayerHarry", foundOn, sizeof(foundOn));
    g_offLightPlayerHarry = off > 0 ? off : -1;
    logf_("[lumos] v70 LumosLight.PlayerHarry %s (+0x%X on %s)",
          off > 0 ? "resolved" : "NOT FOUND", off, off > 0 ? foundOn : "-");
}

// Does the receiver's class chain contain the given class object? Same
// guarded chain walk lumosChainProves uses, for one reference.
static BOOL lumosChainContainsClass(void *obj, void *want)
{
    if (!obj || !want || !g_cgChainOK) return FALSE;
    for (void *c = cgClassOf(obj); c; c = cgSuper(c))
        if (c == want) return TRUE;
    return FALSE;
}

// Does the receiver's class chain reach Engine.Actor? (Never run Trigger or
// hand actor fields to anything that cannot prove this.)
static BOOL lumosChainHasActor(void *obj)
{
    return obj && g_clsActor ? lumosChainContainsClass(obj, g_clsActor) : FALSE;
}

// The most-derived Trigger declaration in the receiver's OWN class chain -
// what the engine's event dispatch would run for this receiver. Never a
// foreign-class function (the v66.4 lesson). v68: a UE2-style Mover declares
// its operative Trigger INSIDE a state (Pkg.Class.State.Trigger, e.g.
// Mover.TriggerToggle.Trigger), which the class-level path can never see -
// the old lookup fell back to the EMPTY Engine.Actor base event and the
// walk counted that no-op as a successful fire. The second pass now also
// admits state-declared Trigger functions, most-derived declaring class
// wins. The full-table scans only run on this fallback path (edge
// triggered), and a per-receiver cache keeps a 1 Hz failing retry from
// re-walking the table every time.
static void *lumosTriggerFnFor(void *obj)
{
    if (!obj) return NULL;
    // v68: per-receiver cache - a failing dispatch retries at 1 Hz, and the
    // Tag walk re-resolves the same receiver every time. Liveness-validated
    // so a level change (freed UFunction) can never return a stale pointer.
    static struct { void *obj; void *fn; } sCache[32];
    for (int i = 0; i < 32; i++) {
        if (sCache[i].obj != obj) continue;
        if (sCache[i].fn && cgLiveObject(sCache[i].fn)) return sCache[i].fn;
        sCache[i].obj = NULL; sCache[i].fn = NULL;
        break;
    }
    void *result = NULL;
    char cbuf[160];
    void *c = cgClassOf(obj);
    for (int d = 0; c && d < 12; d++, c = cgSuper(c)) {
        // v70: stop BEFORE the empty Engine.Actor base - v69 found it here
        // first and the state-declared pass below therefore never ran
        // (log: "Tag walk fires Function Engine.Actor.Trigger on
        // LumosSparklesTrigger5 ... EMPTY base - no-op!"). The base is
        // re-added as the very last resort after the state pass.
        if (g_clsActor && c == g_clsActor) break;
        objName(c, cbuf, sizeof(cbuf));
        const char *sp = strchr(cbuf, ' ');
        if (!sp) break;
        char path[192];
        sprintf(path, "%s.Trigger", sp + 1);
        void *fn = findObjectByPath(path);
        if (fn) { result = fn; break; }
    }
    // State-declared Trigger: Pkg.Class.<State>.Trigger, declared somewhere
    // in the receiver's chain. One table pass, most-derived class first.
    if (!result && g_objArray && g_objArray->Data) {
        int n = g_objArray->Num;
        if (n > 0 && n <= 400000 && !IsBadReadPtr(g_objArray->Data, 4)) {
            char nb[256];
            for (c = cgClassOf(obj); c && !result; c = cgSuper(c)) {
                objName(c, cbuf, sizeof(cbuf));
                const char *sp = strchr(cbuf, ' ');
                if (!sp) break;
                int plen = (int)strlen(sp + 1);
                for (int k = 0; k < n; k++) {
                    void *o = g_objArray->Data[k];
                    if (!o || IsBadReadPtr(o, 0x100)) continue;
                    objName(o, nb, sizeof(nb));
                    const char *nsp = strchr(nb, ' ');
                    if (!nsp) continue;
                    const char *full = nsp + 1;
                    if (strncmp(full, sp + 1, plen)) continue;  // declaring prefix
                    if (full[plen] != '.') continue;            // exact boundary
                    const char *rest = full + plen + 1;
                    if (!strchr(rest, '.')) continue;           // class-level only
                    if (strcmp(rest + strlen(rest) - 8, ".Trigger")) continue;
                    result = o;                                 // <State>.Trigger
                    break;
                }
            }
        }
    }
    // v70: last resort - the empty base (the caller recognises it via
    // g_fnActorTrigger and treats it as a NON-fire).
    if (!result) result = g_fnActorTrigger;
    // Store (even a NULL - the failure is stable for a live receiver).
    for (int i = 0; i < 32; i++) {
        if (!sCache[i].obj || (sCache[i].fn && !cgLiveObject(sCache[i].fn))) {
            sCache[i].obj = obj;
            sCache[i].fn = result;
            break;
        }
...
// The stock event dispatch, done by hand: every live actor whose Tag equals
// the trigger's Event gets its own Trigger(Other, Instigator). This is exactly
// what Engine.Actor.TriggerEvent does - the fallback for engines without the
// exec export. Edge-triggered (once per level per trigger), so the full-table
// walk is fine here; it never runs while Lumos is off.
// v68: a receiver whose OWN chain declares no Trigger (the only candidate
// left would be the EMPTY Engine.Actor base event) is SKIPPED, not called -
// v67 counted that guaranteed no-op as a fire, latched the once-per-level
// flag and suppressed every later real dispatch attempt.
// v70: (1) an EMPTY Engine.Actor.Trigger receiver is NOT a fire; (2) such a
// receiver is a RELAY - its own Event is followed (depth-limited), because
// the HP3 secret-wall chain the v69 log exposed is
//   LumosTrigger --Event--> LumosSparklesTrigger --Event--> wall/mover
// (the 4 Event==Tag pairings at scan time were sparkles.Event == wall.Tag);
// a receiver whose Trigger IS scripted re-broadcasts on its own (stock
// Mover/Trigger do it inside their Trigger) and is NOT relayed again;
// (3) every receiver is NAMED in the log.
static BOOL lumosDispatchByTagDepth(DWORD ev, void *other, void *instigator, int depth);
static BOOL lumosDispatchByTag(DWORD ev, void *other, void *instigator)
{
    return lumosDispatchByTagDepth(ev, other, instigator, 0);
}
static BOOL lumosDispatchByTagDepth(DWORD ev, void *other, void *instigator, int depth)
{
    if (!g_objArray || !g_objArray->Data || g_offTag <= 0 || !ev) return FALSE;
    if (depth > 3) return FALSE;
    int n = g_objArray->Num;
    if (n <= 0 || n > 400000 || IsBadReadPtr(g_objArray->Data, 4)) return FALSE;
    int fired = 0;
    char fnName[160], rn[160];
    for (int k = 0; k < n; k++) {
        void *o = g_objArray->Data[k];
        if (!o || o == other || IsBadReadPtr(o, 0x100)) continue;
        if (IsBadReadPtr((BYTE *)o + g_offTag, 4)) continue;
        if (*(DWORD *)((BYTE *)o + g_offTag) != ev) continue;
        if (!lumosChainHasActor(o)) continue;      // never a Texture/UClass/Function
        if (cgDeleted(o)) continue;
        objName(o, rn, sizeof(rn));
        void *fn = lumosTriggerFnFor(o);
        BOOL emptyBase = (!fn || (g_fnActorTrigger && fn == g_fnActorTrigger));
        if (!emptyBase) {
            BYTE p[16]; memset(p, 0, sizeof(p));
            *(void **)(p + 0) = other;
            *(void **)(p + 4) = instigator;
            objName(fn, fnName, sizeof(fnName));
            logf_("  [lumos] v70 Tag walk[%d] fires %s on %s (own Trigger)",
                  depth, fnName, rn);
            callFnP(o, fn, p, 8, "Trigger(Event)");
            fired++;
        } else {
            logf_("  [lumos] v70 Tag walk[%d] receiver %s has only the EMPTY "
                  "Engine.Actor.Trigger - relaying its own Event", depth, rn);
        }
        // Relay: follow the receiver's own Event (LumosSparklesTrigger ->
        // wall). Stock LumosTrigger.StateLumosOn does exactly this on the
        // sparkles, and the sparkles' Trigger (if any) does it onward.
        if (emptyBase && g_offEvent > 0 && !IsBadReadPtr((BYTE *)o + g_offEvent, 4)) {
            DWORD ev2 = *(DWORD *)((BYTE *)o + g_offEvent);
            if (ev2 && ev2 != ev &&
                lumosDispatchByTagDepth(ev2, o, instigator, depth + 1))
                fired++;
        }
    }
    return fired > 0;
...
// v70: relay an Event one hop through every Tag-matched receiver's OWN Event
// using the script TriggerEvent on that receiver (depth-limited, receivers
// named in the log). This is how HP3's LumosTrigger -> LumosSparklesTrigger
// -> wall linkage reaches the wall when the sparkles' Trigger is the empty
// base (it re-broadcasts nothing). Read-only over the table, one
// ProcessEvent per relaying receiver.
static void lumosRelayEventChain(DWORD ev, void *src, void *instigator, int depth)
{
    if (depth > 3 || !ev || !g_fnActorTriggerEvent || !g_ProcessEvent) return;
    if (!g_objArray || !g_objArray->Data || g_offTag <= 0 || g_offEvent <= 0) return;
    int n = g_objArray->Num;
    if (n <= 0 || n > 400000 || IsBadReadPtr(g_objArray->Data, 4)) return;
    char rn[160];
    for (int k = 0; k < n; k++) {
        void *o = g_objArray->Data[k];
        if (!o || o == src || IsBadReadPtr(o, 0x100)) continue;
        if (IsBadReadPtr((BYTE *)o + g_offTag, 4)) continue;
        if (*(DWORD *)((BYTE *)o + g_offTag) != ev) continue;
        if (!lumosChainHasActor(o) || cgDeleted(o)) continue;
        objName(o, rn, sizeof(rn));
        DWORD ev2 = 0;
        if (!IsBadReadPtr((BYTE *)o + g_offEvent, 4))
            ev2 = *(DWORD *)((BYTE *)o + g_offEvent);
        void *fn = lumosTriggerFnFor(o);
        BOOL emptyBase = (!fn || (g_fnActorTrigger && fn == g_fnActorTrigger));
        logf_("  [lumos] v70 event receiver[%d]: %s (own Trigger: %s, Event=0x%X)",
              depth, rn, emptyBase ? "EMPTY base" : "scripted", ev2);
        // Relay only through receivers that cannot re-broadcast themselves
        // (empty Trigger). A scripted receiver (Mover, Trigger) already did
        // whatever stock does with its own Event.
        if (!emptyBase || !ev2 || ev2 == ev) continue;
        BYTE parms[16]; memset(parms, 0, sizeof(parms));
        *(DWORD *)(parms + 0) = ev2;
        *(void **)(parms + 4) = o;
        *(void **)(parms + 8) = instigator;
        logf_("  [lumos] v70 relaying %s.Event=0x%X via script TriggerEvent",
              rn, ev2);
        g_ProcessEvent(o, NULL, g_fnActorTriggerEvent, parms, NULL);
        lumosRelayEventChain(ev2, o, instigator, depth + 1);
    }
}

// Fire one cached LumosTrigger's Event the way stock's StateLumosOn does,
// once per level, honoring the stock bFirstEventSent latch both ways.
// v68 dispatch order: the ENGINE NATIVE first - it is the stock call and it
// resolves the receiver's operative Trigger correctly (including
// state-declared ones). The manual Tag walk is only the fallback for
// engines without the export. v67's walk-first order could count a no-op
// (empty base Trigger) as a fire, latch the slot and suppress the native
// forever - the wall stayed solid while the log claimed success. A failed
// fire no longer latches at all: the caller retries at 1 Hz.
static void lumosFireTriggerEvent(int slot, void *tr, void *instigator)
{
    if (!tr || slot < 0 || slot >= LUMOS_MAX_ACTORS) return;
    if (g_lumosTrigFired[slot] == 1) return;
    if (g_offEvent <= 0) return;   // field not resolved yet: caller retries
    if (IsBadReadPtr((BYTE *)tr + g_offEvent, 4)) return;
    DWORD ev = *(DWORD *)((BYTE *)tr + g_offEvent);
    if (!ev) { g_lumosTrigFired[slot] = 1; return; }   // nothing linked: final
    BOOL sent = FALSE;
    const char *how = "none";
    // v70 PRIMARY: the SCRIPT Engine.Actor.TriggerEvent(EventName, Other,
    // Instigator) via ProcessEvent ON THE TRIGGER ITSELF - byte for byte
    // what stock LumosTrigger.StateLumosOn.BeginState runs
    // (TriggerEvent(Event, self, None)). The engine then performs the
    // virtual Trigger dispatch on every Tag-matched receiver, state-declared
    // Mover Triggers included - the part the mod's own Tag walk can never do
    // (v69 log: walk hit the EMPTY base on LumosSparklesTrigger5, wall stayed
    // shut). It cannot report whether any receiver existed, so a receiver
    // count from the (call-free) Tag census decides "sent".
    if (g_fnActorTriggerEvent && g_ProcessEvent) {
        BYTE parms[16]; memset(parms, 0, sizeof(parms));
        *(DWORD *)(parms + 0) = ev;
        *(void **)(parms + 4) = tr;
        *(void **)(parms + 8) = instigator;
        g_ProcessEvent(tr, NULL, g_fnActorTriggerEvent, parms, NULL);
        sent = TRUE;
        how = "script Engine.Actor.TriggerEvent (engine virtual dispatch)";
        // The sparkles relay: stock's sparkles receiver has an EMPTY Trigger
        // and re-broadcasts nothing by itself in the HP2 lineage - the wall
        // there is linked to the LumosTrigger directly. HP3 links it through
        // the sparkles (scan: 4 walls paired by sparkles.Event == wall.Tag),
        // so relay the sparkles' own Event through the same script call.
        lumosRelayEventChain(ev, tr, instigator, 1);
    }
    if (!sent && g_execTriggerEvent && g_opsOK) {
        sent = nativeTriggerEvent(tr, ev, tr, instigator);
        how = "native execTriggerEvent";
    }
    if (!sent) {
        sent = lumosDispatchByTag(ev, tr, instigator);
        how = "manual Tag walk";
    }
    if (sent) {
        g_lumosTrigFired[slot] = 1;
        // Stock sets bFirstEventSent so its own state machine never fires the
        // same event again - a second fire on a TriggerToggle wall would
        // toggle it CLOSED. Mark it for the same reason.
        lumosWriteBit(tr, g_offTrigFirstSent, g_maskTrigFirstSent, TRUE);
        char tn[160]; objName(tr, tn, sizeof(tn));
        char in[96] = "?";
        if (instigator) objName(instigator, in, sizeof(in));
        logf_("  [lumos] v68 TriggerEvent fired (%s): %s event=0x%X "
              "instigator=%s (wall opens - stock fires this once per level)",
              how, tn, ev, in);
    } else {
        // No dispatch path or no receiver found: do NOT latch - the wall
        // linkage may still appear. The caller throttles the retry.
        g_lumosTrigNextTryAt[slot] = GetTickCount() + hp3lumos::DispatchRetryMs;
        if (!g_lumosTrigWarned) {
            g_lumosTrigWarned = TRUE;
            char tn[160]; objName(tr, tn, sizeof(tn));
            logf_("  [lumos] v68 no TriggerEvent dispatch yet (execTriggerEvent="
                  "%p) for %s event=0x%X - retrying at 1 Hz",
                  g_execTriggerEvent, tn, ev);
        }
    }
}
...
// v68: last time an empty scan result was accepted; the next quiet 1 Hz
// retry is allowed once this is EmptyScanRetryMs old.
static DWORD g_lumosEmptyScanAt = 0;
...
// v68: the trigger slot each wall is paired with at scan time (-1 = none).
// The wall-open proximity test measures from the PAIRED TRIGGER (stock's
// own InLumosRadius check), because a large wall's own Location can sit far
...
static BOOL lumosChainProves(void *obj, BOOL wantWall)
{
    ...
    // v69: triggers prove against the two known family classes OR the
    // Engine.Triggers anchor itself (LumosTrigger/LumosSparkles declare
    // `extends Triggers`). The anchor alone is never sufficient on its own:
    // this helper is only consulted for objects whose NAME already matched
    // a Lumos/wall look-alike token (see lumosScanActors), so an ordinary
    // Trigger/Distances/Specials actor can never reach this test. It admits
    // HP3-only placed subclasses such as a "LumosTriggerLarge" the exact
    // name rule must reject.
    void *refs[3];
    int refsN;
    if (wantWall) {
        refs[0] = g_clsGenericColObj;
        refs[1] = g_clsKWBlockingVolume;
        refs[2] = NULL;
        refsN = 2;
    } else {
        refs[0] = g_clsLumosTrigger;
        refs[1] = g_clsLumosSparklesTrigger;
        refs[2] = g_clsTriggersFamily;
        refsN = 3;
    }
    if (!refs[0] && !refs[1] && !refs[2]) return FALSE;
...
static void lumosScanActors(BOOL quiet = FALSE)
{
    ...
    // v69: pairing is now FIRST the stock linkage itself - the trigger's
    // Event NAME equals the wall's Tag NAME (TriggerEvent dispatches the
    // Event to every actor whose Tag matches, which is how the stock wall
    // is wired to the stock trigger). Distance pairing (SecretWallPairRadius,
    // 900) stays as the secondary heuristic for walls whose trigger opens
    // them through some other link (e.g. via an intermediary). The pure
    // radius rule classified a large wall whose actor centre sits >900 units
    // from its trigger as ORDINARY, so the SetCollision fallback never even
    // considered it - a silent lockout with no log line.
    int paired = 0, pairedByLink = 0, pairedByDist = 0;
    for (int w = 0; w < g_lumosWallsN; w++) {
        ...
        g_lumosWallTrig[w] = -1;         // no paired trigger yet
        void *wall = g_lumosWalls[w];
        if (!wall || !cgLiveObject(wall)) continue;
        // v69: linkage pairing - wall.Tag == trigger.Event (raw name-index
        // dword compare; both offsets are engine-side and resolved at boot).
        DWORD wallTag = 0;
        if (g_offTag > 0 && !IsBadReadPtr((BYTE *)wall + g_offTag, 4))
            wallTag = *(DWORD *)((BYTE *)wall + g_offTag);
        if (wallTag) {
            for (int t = 0; t < g_lumosTriggersN; t++) {
                void *tr = g_lumosTriggers[t];
                if (!tr || !cgLiveObject(tr) || g_offEvent <= 0 ||
                    IsBadReadPtr((BYTE *)tr + g_offEvent, 4)) continue;
                DWORD ev = *(DWORD *)((BYTE *)tr + g_offEvent);
                if (hp3lumos::wallEventLinksToTrigger(wallTag, ev)) {
                    g_lumosWallSecret[w] = TRUE;
                    g_lumosWallTrig[w] = (short)t;
                    paired++; pairedByLink++;
                    break;
                }
            }
        ...
        if (g_lumosWallSecret[w]) {
            // linked by name: done for this wall
        } else if (F.Location > 0 &&
                   !IsBadReadPtr((BYTE *)wall + F.Location, 12)) {
    ...
    if (quiet && g_lumosTriggersN == 0 && g_lumosWallsN == 0) {
        static BOOL sEmptySaid = FALSE;
        if (!sEmptySaid) {
            sEmptySaid = TRUE;
            logf_("[lumos] scan: no triggers/walls yet - retrying at 1 Hz "
                  "(menu or level still loading; the v67 one-shot scan could "
                  "lock a level out of wall replication)");
        }
        return;
    }
    ...
    // still bumped into something.
    if (!quiet) {
        char tn[160];
        for (int t = 0; t < g_lumosTriggersN && t < 24; t++) {
            void *tr = g_lumosTriggers[t];
            if (!tr || !cgLiveObject(tr)) continue;
            DWORD tag = (g_offTag > 0 && !IsBadReadPtr((BYTE *)tr + g_offTag, 4))
                        ? *(DWORD *)((BYTE *)tr + g_offTag) : 0;
            DWORD ev  = (g_offEvent > 0 && !IsBadReadPtr((BYTE *)tr + g_offEvent, 4))
                        ? *(DWORD *)((BYTE *)tr + g_offEvent) : 0;
            float *tl = (F.Location > 0 && !IsBadReadPtr((BYTE *)tr + F.Location, 12))
                        ? (float *)((BYTE *)tr + F.Location) : NULL;
            logf_("    trigger[%d] %s Tag=0x%X Event=0x%X at (%.0f %.0f %.0f)",
                  t, objName(tr, tn, sizeof(tn)), tag, ev,
                  tl ? tl[0] : 0.f, tl ? tl[1] : 0.f, tl ? tl[2] : 0.f);
        }
        for (int w = 0; w < g_lumosWallsN; w++) {
            if (!g_lumosWallSecret[w]) continue;
            void *wall = g_lumosWalls[w];
            if (!wall || !cgLiveObject(wall)) continue;
            DWORD tag = (g_offTag > 0 && !IsBadReadPtr((BYTE *)wall + g_offTag, 4))
                        ? *(DWORD *)((BYTE *)wall + g_offTag) : 0;
            float *wl = (F.Location > 0 && !IsBadReadPtr((BYTE *)wall + F.Location, 12))
                        ? (float *)((BYTE *)wall + F.Location) : NULL;
            logf_("    secret wall #%d %s Tag=0x%X trig=%d bits=0x%02X at "
                  "(%.0f %.0f %.0f)", w, objName(wall, tn, sizeof(tn)), tag,
                  g_lumosWallTrig[w], g_lumosWallOrig[w],
                  wl ? wl[0] : 0.f, wl ? wl[1] : 0.f, wl ? wl[2] : 0.f);
        }
    }
}
...
static void *lumosLightOf(int p, void *pawn)
{
    ...
    // v67: TheLumosLight resolved on the wand's OWN class (chain-walked by
    // lumosLightPropOffForWand) - the exact stock field the wand's Tick reads.
    int offDyn = lumosLightPropOffForWand(wand);
    if (offDyn > 0 && !IsBadReadPtr((BYTE *)wand + offDyn, 4)) {
        void *l = *(void **)((BYTE *)wand + offDyn);
...
static void lumosTick(void)
{
    ...
    // of handing GetFullName a freed pointer. v68: a scan that found nothing
    // (the MAIN MENU is the usual case - lumosTick also runs there) is no
    // longer final: it retries quietly at 1 Hz until the level's actors
    // appear, so a first level entered without a camera-package invalidation
    // can no longer be locked out of wall replication forever.
    if (!g_lumosScanDone)
        lumosScanActors();
        ...
        lumosInvalidate(); lumosScanActors();
    } else if (g_lumosTriggersN == 0 && g_lumosWallsN == 0 &&
               (DWORD)(now - g_lumosEmptyScanAt) >= hp3lumos::EmptyScanRetryMs) {
        g_lumosEmptyScanAt = now;
        lumosScanActors(TRUE);
    }
    ...
    // v67: the brightness read is type-aware - on a float-brightness engine
    // the old byte read saw 0x00 inside 400.0f and never engaged.
    for (int p = 0; p < numP; p++) {
        void *pawn = pw[p];
        if (!pawn) continue;
        void *light = lumosLightOf(p, pawn);
        if (!light) continue;
        BYTE lt = lumosReadByte(light, g_offLightType, 1);
        float br = lumosBrightnessOf(light);
        if (hp3lumos::isLightActiveF(false, lt, br)) {
            if (!g_lumosState.active) {
                g_lumosState.activate(p, now, hp3lumos::DefaultLumosDurationMs);
                // v68: fresh window - re-arm every player's replicated light.
                lumosClearDeadLatches();
                logf_("  [lumos] active light found on P%d - state engaged (30s)", p);
            } else {
    ...
    // ~1 s after the light dies - exactly the window in which v67 re-lit
    // the light and the Lumos never ended). A FRESH cast re-enters through
    // the inactive->active transition, which clears every latch.
    ...
    if (lumosActive) {
        for (int p = 0; p < numP; p++) {
            ...
            // v69: stock baseWand.Tick only computes WandEndPoint - the
            // point UpdateLocation rides the light AND its Particles onto -
            // inside `if ((Pawn(Owner) != None) && (Pawn(Owner).Weapon ==
            // self))`. A companion whose Weapon slot is empty makes the
            // stock ride write the (unassigned, zero) local vector over the
            // light and glow position every frame - the glow ends up parked
            // at the world origin and the wand never visibly glows. Fill an
            // EMPTY Weapon slot with the companion's own wand actor once;
            // never overwrite an existing Weapon.
            if (p > 0 && wand && g_offWeapon > 0 &&
                !IsBadReadPtr((BYTE *)pawn + g_offWeapon, 4)) {
                void *curW = *(void **)((BYTE *)pawn + g_offWeapon);
                if (!curW) {
                    *(void **)((BYTE *)pawn + g_offWeapon) = wand;
                    logf_("  [lumos] v69 P%d empty Weapon slot filled with "
                          "its wand (stock baseWand.Tick ride target now "
                          "valid)", p);
                } else if (curW != wand) {
                    // Occupied by something else: the stock ride target is
                    // then wrong for this wand (Weapon==self fails and the
                    // ride zeroes light+glow). Report it once instead of
                    // silently overwriting a live weapon slot.
                    static BOOL sWeaponSaid[8] = { FALSE, FALSE, FALSE, FALSE,
                                                   FALSE, FALSE, FALSE, FALSE };
                    if (!sWeaponSaid[p]) {
                        sWeaponSaid[p] = TRUE;
                        char nb[128];
                        logf_("  [lumos] v69 P%d Weapon slot holds %s (not "
                              "its wand) - stock ride target mismatch",
                              p, cgLiveObject(curW)
                                 ? objName(curW, nb, sizeof(nb))
                                 : "<dead ptr>");
                    }
                }
            }
            void *light = lumosLightOf(p, pawn);
            // v70: resolve the pawn bit / light gate var from the live
            // objects' own class chains (the fixed-path guesses all missed).
            ...
            // the one-shot and touch nothing - rewriting the register or
            // destroying an already-destroyed glow would only fight the
            // game. The next cast re-arms through the fresh-window latch
            // clear.
            // v70: ONLY for stock-lit lights. The v68 fallback register
            // deliberately holds bLumosOn low until a glow exists (and with
            // no glow class on HP3 - v69 log GlowFX=0 - it stayed low), so
            // this latch read the fallback light as "cold" ONE FRAME after
            // lighting it and killed the whole cast (v69 log: "turned ON
            // (v68 fallback register)" immediately followed by "went cold on
            // its own"). A fallback light is timed by the mod, below.
            if (p > 0 && hp3lumos::observedStockAutoOff(g_lumosLightOn[p] != FALSE,
            ...
            if (p > 0 &&
                ...
                // replication hand-rolled - TurnDynamicLightOn register
                // (LT_Steady, LE_NonIncidence, brightness, hue 32,
                // saturation 72, radius 15, radiusInner 5), Enable('Tick')
                // (so the STOCK 30 s auto-off, particle scaling and
                // OnLumosOff broadcast run stock-side), the AllActors
                // OnLumosOn broadcast (arms the secret-wall triggers), and
                ...
                // in LumosLight.PreBeginPlay from Level.PlayerHarryActor;
                // by sync time the gargoyle already lit the lead, so the
                // gate would early out). The gate dance: mask the lead's
                // bLumosOn bit for the duration of ONE synchronous
                // ProcessEvent call on the game thread - no other script can
                // run mid-call - then restore every pawn's bit. TurnOn
                // re-asserts PlayerHarry.bLumosOn = True itself anyway.
                if (g_fnLumosTurnOn && g_ProcessEvent) {
                    // v70 GATE: the v69 dance cleared the pawns' bLumosOn
                    // bits, but that bit never resolved on HP3 (log:
                    // Pawn.bLumosOn=+0xFFFFFFFE) so nothing was masked, the
                    // lead's bit stayed True and stock TurnOn early-returned
                    ...
                    if (g_offLightPlayerHarry > 0 && lumosPawnHasLumosBit(pawn) &&
                        ...
                        *(void **)((BYTE *)light + g_offLightPlayerHarry) = pawn;
                        phSwapped = TRUE;
                    }
                    BOOL  saved[8];
                    for (int q = 0; q < numP; q++) {
                        saved[q] = FALSE;
                        if (!pw[q] || g_offPawnLumosOn <= 0 || !g_maskPawnLumosOn) continue;
                        if (phSwapped && q != p) continue;   // only the gate pawn matters
                        if (!lumosPawnHasLumosBit(pw[q])) continue;
                        saved[q] = lumosReadBit(pw[q], g_offPawnLumosOn,
                                                g_maskPawnLumosOn, FALSE);
                        lumosWriteBit(pw[q], g_offPawnLumosOn,
                                      g_maskPawnLumosOn, FALSE);
                    }
                    callFn(light, g_fnLumosTurnOn,
                           "LumosLight.TurnOn [stock replication]");
                    if (phSwapped)
                        *(void **)((BYTE *)light + g_offLightPlayerHarry) = savedPH;
                    // Restore ONLY the bits we cleared of TRUE (a pawn whose
                    // saved bit was FALSE is left exactly as stock left it).
                    // The gate pawn itself is set TRUE regardless - stock
                    // wrote it if the call engaged, and it is the mod's own
                    // replicated state either way.
                    for (int q = 0; q < numP; q++) {
                        if ((saved[q] || q == p) && pw[q] && g_offPawnLumosOn > 0 &&
                            g_maskPawnLumosOn && lumosPawnHasLumosBit(pw[q]))
                            lumosWriteBit(pw[q], g_offPawnLumosOn,
                                          g_maskPawnLumosOn, TRUE);
                    }
                    if (lumosLightBurning(light)) {
                    ...
                    // unreadable -> PlayerHarry ref likely None, or the
                    // call aborted). The v68 fallback below still applies
                    // the register + hand-rolled glow.
                    logf_("  [lumos] v69 P%d stock TurnOn did not engage "
                          "(light still cold; PlayerHarry var %s, pawn bit %s) "
                          "- falling back to the v68 register replication", p,
                          phSwapped ? "retargeted" : "UNRESOLVED",
        ...
        // v67: only lights the MOD turned on are turned off, with the exact
        // stock TurnOff register (stock TurnOff also destroys the particles
        // and writes PlayerHarry.bLumosOn = False). A stock-burning light
        // (the lead's, or an infinite gargoyle's) stays in the game's hands.
        for (int p = 0; p < numP; p++) {
            void *pawn = pw[p];
            // v70: companions only (the lead's bit belongs to the stock
            // chain) and only where the class declares the bit.
            if (p > 0 && pawn && g_offPawnLumosOn > 0 && g_maskPawnLumosOn &&
                lumosPawnHasLumosBit(pawn))
    ...
    // -----------------------------------------------------------------------
    // v67: THE WALL-OPEN PATH STOCK ACTUALLY USES. Stock LumosTrigger arms on
    // the OnLumosOn broadcast (which the game runs stock-side whenever any
    // player's Lumos lights a gargoyle - HandleSpellLumos hardcodes
    // PlayerHarry's wand), then fires TriggerEvent(Event, self, None) ONCE
    // when PlayerHarry comes within its own fDistanceCheck. That check is the
    // only thing a companion cannot satisfy - so the mod replicates exactly
    // it: Lumos on, trigger's own Event non-empty, nobody has fired it yet
    ...
    // hardware pelog of the v66.4 field report showed the most-derived
    // Trigger on a placed LumosTrigger is Engine.Actor.Trigger, the empty
