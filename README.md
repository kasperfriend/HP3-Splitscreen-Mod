...
- Real spellcasting through the game's own script VM (Rictusempra, Depulso, Lumos, ... ) with hold-to-aim, vertical aim, and v50 gameplay activation for castable objects/SpellTriggers
...
> Check the build: open `system\hp3mod.log` — line 2 must say `build v50`.
...
The floor-handoff policy tests pass; **v49/v50 still need in-game hardware confirmation**. If a hop remains, send the new log rather than increasing the old suppression timers. For cast gameplay, leave `[actions] CastGameplay=1` and check for `[castgame] target` / `activating` lines when aiming at a spell object.
...
4. Optionally enter a release **tag** and **title**, then run the job. Leave the tag blank to auto-generate a unique one (for example `v50-20260908-1234567890`).
...
src/dllmain.cpp      mod / engine integration (v50)
...
bin/d3d8.dll         prebuilt v49 (not rebuilt in this sandbox; source is v50)
