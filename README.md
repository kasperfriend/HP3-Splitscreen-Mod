...
- **Cooperative sustained-cast (v57)**: if **any** character — Player 1's engine cursor lock, or a Player 2/Player 3 held cast — keeps the same game-verified cast target (the game's own `vulnerableToClass`/spell-handler targeting metadata, or the stock cursor lock) for **more than 10 seconds**, the real Harry/Hermione/Ron enter the shared hold: the trio gets the target's own spell and held-cast state, P1's real `SpellCursor.LockOn` bridges the engine's hover-lock when its ABI reflects safely, heroes the game already recruited itself are left untouched, and everything is restored on release. Target flicker up to 250 ms never resets the dwell. Also fixes the hard crash (GPF in `UObject::GetPathName <- GetFullName`) when entering Hogwarts / changing levels: cached actors are validated against the live object table before any name or field use, and the level-change cache drop runs every frame.
...
### v57 cooperative sustained cast (any character's >10s hold; v55/v56 certification findings folded in)
...
v57 applies that same effect when **any single character** holds the same target for more than 10 seconds:

- **any character, any mode**: Player 1's engine cursor lock *or* a Player 2/3 held cast on the same game-verified cast target (a living candidate with real `vulnerableToClass`/spell-handler/SpellTrigger metadata — exactly what the engine's own cursor accepts — or the stock cursor lock itself) arms the shared hold after `CoopCastHoldMs`. The v54–v56 behaviour-certification gate proved structurally unreachable on hardware (see FINDINGS §16: the stock companion join does not set both companions' pawn `spellTarget`, and a split-ON holder is never the player whose cursor the cert watched) — it now only enriches discovery of cooperative actors with no targeting metadata and can never block a hold.
- **the trio is borrowed through the engine's own state**: the verified target/spell fields plus `StartCasting(class, 1.0)`/`PressedFire`/`playCastAim` on the non-holders, and the genuine P1 `SpellCursor.LockOn` when reflection proves its one-target ABI and the paired no-input `UnLock` (best-effort in v57 — a level without a safely reflected cursor pair no longer cancels the whole borrow). Heroes the game itself already recruited on that target are neither restarted nor force-stopped.
- **flicker tolerance**: up to 250 ms of lost target (cursor `None` frames for P1, aim-pick gaps for P2/P3) does not reset any holder's dwell; a real release re-arms.
